...
| **Onboard speed ("can you add a bridge in a day?")** | No. The env registry (`VITISH_EXTRA_BRIDGES`) proves the SOFTWARE path is a **same-run exercise** — a second bridge streams, fuses to its own BHI, persists and shows on the map with zero code change. But onboarding a REAL bridge is a **days-scale** task: sensor + edge-node install, baseline/calibration data, channel-model fit, registry + config (`/api/config.multi_bridge.onboard_label`). The demo extra bridge is **simulated telemetry (synthetic channel model), never a real sensor**; its coordinates are schematic unless supplied. |
