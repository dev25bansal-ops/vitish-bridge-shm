...
| 2:00–4:00 | **LIVE DEMO (hero flow)** | Real-time: a crack is detected by the vision model → vibration anomaly score rises → BHI drops from 87 → dashboard flags an alert → the digital twin highlights the affected section. **Label every source on screen** ("Z24 benchmark data · 100 Hz", "simulated feed", "LIVE" badge on the real ESP32 stream if it passes smoke test). Show the confusion matrices ([[Metrics]]): the demo healthy-vs-rupture one is clean (precision/recall 1.0); the honest per-scenario Z24 one shows measured recall ~0.002 — subtle stages do NOT separate on vibration alone, which is exactly why BHI fuses CV + load. End with the rule-based copilot: "Tendon-rupture signature detected — recommend load restriction and strain-gauge verification." |
