...
a SMALL, deterministic slice of the real benchmark — 120 windows (1024 samples,
channels 6/7/8) per group, real float32 Z24 data — and commits it under
`data/z24/fixture/` (~1.5 MB) so the gates exercise real Z24 evidence everywhere.

Regenerate (only when you have the full file) with:

    python scripts/make_z24_fixture.py --n-seg 8
...
def main() -> int:
    ...
    ap.add_argument("--n-seg", type=int, default=8,
                    help="segments sampled per label (default 8 -> 120 windows)")
    args = ap.parse_args()
